LINKS: 
    Icons: www.flaticon.com/free-icons/
        PDF: Pdf icons created by Prashu Rapol - Flaticon

        IMG: https://www.flaticon.com/free-icons/photo,  Photo icons created by Freepik - Flaticon

        .DOC(X): Document icons created by Freepik - Flaticon

        .XL(M): Excel icons created by Driss Lebbat - Flaticon

        .HTML: Html icons created by kliwir art - Flaticon

        CSS: Css icons created by Pixel perfect - Flaticon

        JS: Javascript icons created by Freepik - Flaticon

        DEFAULT: Document icons created by Freepik - Flaticon

        Stop Icon (Upload): Stop sign icons created by Freepik - Flaticon

        Plus Icon: https://www.flaticon.com/free-icons/add,add icons">Add icons created by Pixel perfect - Flaticon

        generic profile: Gemini AI